# Examples

We host a number of example scripts.

## Perform language analysis

- [Juman++](./apply_jumanpp.py)
- [KNP](./apply_knp.py)
- [KWJA](./apply_kwja.py)

## Load language analysis results

- [Juman++](./load_jumanpp.py)
- [KNP](./load_knp.py)
- [KWJA](./load_knp.py)

## Use language analysis results

- [Morphological analysis](./use_morphological_analysis.py)
- [Dependency parsing](./use_dependency_parsing.py)
- [Named entity recognition](./use_named_entity_recognition.py)
- [Discourse relation analysis](./use_discourse_relation_analysis.py)
- [Predicate-argument structure analysis](./use_predicate_argument_structure_analysis.py)
