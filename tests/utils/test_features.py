...
cases = [
    FeaturesTestCase(
        ...
        features={"BGH": "構文/こうぶん", "文節内": True, "係": "文節内", "文頭": True, "体言": True, "名詞項候補": True,
                  "先行詞候補": True, "正規化代表表記": "構文/こうぶん"},
        length=8,
    )
]
...
def test_features_from_fstring(fstring: str, features: str, length: int):
    fs = Features(fstring)
...
@pytest.mark.parametrize("fstring,features,length", [astuple(case) for case in cases])
def test_features_to_fstring(fstring: str, features: str, length: int):
    fs = Features(fstring)
