...
"""
    ...
    assert repr(argument) == f"Argument(phrase={repr(phrase)}, arg_type={repr(ArgumentType.OMISSION)})"
    assert str(argument) == phrase.text
    assert argument != "test"
    assert argument == Argument(phrase, arg_type=ArgumentType.EXOPHOR)  # TODO: consider whether this is expected


def test_special_argument():
    ...
    assert argument != "test"
    assert argument == SpecialArgument("不特定:人", eid=1)  # TODO: consider whether this is expected
